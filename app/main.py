...
def _safe_optional_decimal(raw: str) -> Decimal | None:
    """Wie ``_optional_decimal``, liefert bei ungültiger Eingabe aber ``None``."""
    try:
...
def create_fixed_asset_action():
    ...
    try:
        ...
        degressive_rate = _optional_decimal(request.form.get("degressive_rate", ""))
        total_units = _optional_decimal(request.form.get("total_units", ""))
        residual_value = _optional_decimal(request.form.get("residual_value", "")) or Decimal(
            "0.00"
        )
    except ValueError:
...
def depreciate_fixed_asset_action(asset_id: int):
    ...
    units = _safe_optional_decimal(request.form.get("units", ""))
    ...
    with session_factory() as session:
        ...
        if asset is None:
            abort(404)
        _require_company_access(session, asset.company_id)
...
def dispose_fixed_asset_action(asset_id: int):
    company_id = request.form.get("company_id", type=int)
    proceeds = _safe_optional_decimal(request.form.get("proceeds", ""))
